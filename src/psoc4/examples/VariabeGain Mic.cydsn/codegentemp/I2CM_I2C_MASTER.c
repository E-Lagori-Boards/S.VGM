...
#else
    #define I2CM_I2CMasterDisableEcAm()     do{ /* Empty */ }while(0)
#endif /* (!I2CM_CY_SCBIP_V0) */

static uint32 I2CM_I2CMasterHandleStatus(uint32 status);
static uint32 I2CM_I2CMasterWaitOneUnit(uint32 *timeout);
...
uint32 I2CM_I2CMasterWriteBuf(uint32 slaveAddress, uint8 * wrData, uint32 cnt, uint32 mode)
{
    ...
    if(I2CM_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        I2CM_SetMasterInterruptMode    (I2CM_I2C_INTR_MASTER_MASK);
...
{
    uint32 errStatus = I2CM_I2C_MSTR_NOT_READY;
    ...
    if(I2CM_CHECK_I2C_FSM_IDLE)
    {
        ...
        {
            uint32 locStatus;
            uint32 timeout = I2CM_I2C_CONVERT_TIMEOUT_TO_US(timeoutMs);

            /* Non-empty for master-slave mode when wakeup enabled */
            I2CM_I2CMasterDisableEcAm();

            /* Clean up the hardware before a transaction. */
            I2CM_SetMasterInterruptMode    (I2CM_NO_INTR_SOURCES);
            I2CM_ClearMasterInterruptSource(I2CM_INTR_MASTER_ALL);
            ...
            do
            {
                locStatus  = I2CM_GetMasterInterruptSource() & I2CM_INTR_MASTER_SEND_BYTE;
                locStatus |= I2CM_GetSlaveInterruptSource() & I2CM_SLAVE_INTR_I2C_ADDR;
                locStatus |= I2CM_I2CMasterWaitOneUnit(&timeout);
            }
            while (0u == locStatus);

            /* Handle completion event and convert to status */
            errStatus = I2CM_I2CMasterHandleStatus(locStatus);
        }
...
* \param bitRnW
* Direction of the following transfer. It is defined by read/write bit within
* address byte.See I2CM_I2CMasterSendStart() function for constants.
*
* \param timeoutMs
* Defines in milliseconds the time that this function can block for.
* If that time expires, the function returns. If a zero is passed,
...
* \return
* Error status.
* See I2CM_I2CMasterSendStart() function for constants.
*
* \sideeffect
...
uint32 I2CM_I2CMasterSendStop(uint32 timeoutMs)
{
    ...
    if(I2CM_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        do
        {
            locStatus  = I2CM_GetMasterInterruptSource() & I2CM_INTR_MASTER_SEND_STOP;
            locStatus |= I2CM_I2CMasterWaitOneUnit(&timeout);
        }
        while (0u == locStatus);
...
* I2CM_I2CMasterSendStop() or
* I2CM_I2CMasterSendRestart() function appropriately.
* This function is blocking. It does not return until a byte is received or
* an error occurs.
*
* \param ackNack
* Response to received byte.
*  - I2CM_I2C_ACK_DATA - Generates ACK.
*     The master notifies slave that transfer continues.
*  - I2CM_I2C_NAK_DATA - Prepares to generate NAK.
...
uint32 I2CM_I2CMasterReadByte(uint32 ackNack, uint8 *rdByte, uint32 timeoutMs)
{
    ...
    if(I2CM_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        do
        {
            locStatus  = I2CM_GetMasterInterruptSource() & I2CM_INTR_MASTER_RECEIVE_BYTE;
            rxNotEmpty = I2CM_GetRxInterruptSource() & I2CM_INTR_RX_NOT_EMPTY;
            locStatus |= I2CM_I2CMasterWaitOneUnit(&timeout);
        }
        ...
        if (0u != rxNotEmpty)
        {
            *rdByte = (uint8)I2CM_RX_FIFO_RD_REG;
            I2CM_ClearRxInterruptSource(I2CM_INTR_RX_ALL);
        }
        ...
        if (I2CM_I2C_MSTR_NO_ERROR == errStatus)
        {
            if(I2CM_I2C_ACK_DATA == ackNack)
            {
...
*  I2CM_I2CMasterReadBuf() function.
*
* \return
...
*  I2CM_I2C_MSTAT_ERR_ARB_LOST or
*  I2CM_I2C_MSTAT_ERR_BUS_ERROR occurred while read transfer.
*
* \globalvars
*  I2CM_mstrRdBufIndex - used to current index within master read
*  buffer.
...
void I2CM_I2CReStartGeneration(void)
{
    ...
    * In case of timeout the I2CM_I2CMasterSendRestart handles this
    * case using own timeout and execute appropriate recovery.
    */

    const uint16 oneUnit = 1u;
    uint32 timeout = I2CM_I2C_PHASE_GEN_TIMEOUT;

    /* Store direction of transfer */
    uint32 readDirection = I2CM_I2C_STATUS_REG & I2CM_I2C_STATUS_M_READ;

    /* Wait until slave releases SDA line: SCL tHIGH is complete */
    while ((I2CM_WAIT_SDA_SET_HIGH) && (timeout > 0u))
    {
        CyDelayUs(oneUnit);
        --timeout;
    }
    ...
    {
        /* Refill timeout */
        timeout = I2CM_I2C_PHASE_GEN_TIMEOUT;
        ...
        if (0u != readDirection)
        {
            ...
            I2CM_SET_I2C_SCL_DR(I2CM_I2C_SCL_LOW);
            CyDelayUs(I2CM_I2C_TLOW_TIME);

            /* Drive tHIGH: SCL high phase */
            I2CM_SET_I2C_SCL_DR(I2CM_I2C_SCL_HIGH);
            CyDelayUs(I2CM_I2C_THIGH_TIME);

            /* Wait until slave releases SDA line: SCL tHIGH is complete */
            while ((I2CM_WAIT_SCL_SET_HIGH) && (timeout > 0u))
            {
                CyDelayUs(oneUnit);
                --timeout;
            }
...
/*******************************************************************************
* Function Name: I2CM_I2CMasterHandleStatus
****************************************************************************//**
*
...
* I2CM_I2C_MASTER_TIMEOUT and I2CM_SLAVE_INTR_I2C_ADDR.
*
* \return
* Error status.
* See I2CM_I2CMasterSendStart() function for constants.
...
static uint32 I2CM_I2CMasterHandleStatus(uint32 status)
{
    ...
    if (0u != (status & I2CM_I2C_MASTER_TIMEOUT))
    {
        retStatus  = I2CM_I2C_MASTER_TIMEOUT;
        resetBlock = I2CM_I2C_RESET_ERROR;
    }
    else if (0u != (I2CM_SLAVE_INTR_I2C_ADDR & status))
    {
        /* Abort the master operation, the slave was addressed first */
        retStatus = I2CM_I2C_MSTR_ERR_ABORT_START;

        I2CM_I2C_MASTER_CMD_REG = 0u;

        I2CM_state = I2CM_I2C_FSM_IDLE;
    }
    else if (0u != (status & I2CM_INTR_MASTER_I2C_ARB_LOST))
    {
        retStatus  = I2CM_I2C_MSTR_ERR_ARB_LOST;

        if (I2CM_CHECK_I2C_FSM_ADDR)
        {
            /* If slave is not enabled reset IP, otherwise give it a chance to match address */
            if (0u == (I2CM_I2C_CTRL_REG & I2CM_I2C_CTRL_SLAVE_MODE))
            {
                resetBlock = I2CM_I2C_RESET_ERROR;
            }
    ...
    else if (0u != (status & I2CM_INTR_MASTER_I2C_BUS_ERROR))
    {
        retStatus  = I2CM_I2C_MSTR_ERR_BUS_ERR;
        resetBlock = I2CM_I2C_RESET_ERROR;
    }
    else if (0u != (status & I2CM_INTR_MASTER_I2C_NACK))
    {
        retStatus = I2CM_I2C_MSTR_ERR_LB_NAK;
    }
    else /* Includes: INTR_MASTER_I2C_STOP and INTR_MASTER_I2C_ACK */
    {
        retStatus = I2CM_I2C_MSTR_NO_ERROR;

        if (0u != (status & I2CM_INTR_MASTER_I2C_STOP))
        {
        ...
        else
        {
            ...
            if (I2CM_CHECK_I2C_FSM_ADDR)
            {
                ...
                I2CM_state = (I2CM_CHECK_I2C_FSM_RD) ?
                                          I2CM_I2C_FSM_MSTR_RD_DATA : I2CM_I2C_FSM_MSTR_WR_DATA;
            }
        }
    }

    if (I2CM_I2C_RESET_ERROR == resetBlock)
    {
...
static uint32 I2CM_I2CMasterWaitOneUnit(uint32 *timeout)
{
    ...
    if (*timeout > 0u)
    {
        ...
        if (0u == *timeout)
        {
            status = I2CM_I2C_MASTER_TIMEOUT;
        }
    }
